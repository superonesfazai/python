...
- Celery Worker：执行任务的消费者(任务执行单元), worker并发的运行在分布式系统节点中，通常会在多台服务器运行多个消费者来提高执行效率。
- Broker：消息中间件，接受任务生产者发送过来的任务消息，存进队列再按序分发给任务消费方（通常是消息队列或者数据库）。
- Result Backend：任务处理完后保存状态信息和结果，以供查询。celery本身不提供消息服务，但是可方便和三方提供的消息中间件集成! Celery默认已支持Redis、RabbitMQ、MongoDB、Django ORM、SQLAlchemy等方式。
