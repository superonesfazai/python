...
def run_forever():
    while True:
        ...
        try:
            result = list(tmp_sql_server._select_table(sql_str=mia_select_str_5))
        except TypeError:
        ...
        if result is None:
            ...
            for item in result:
                ...
                if tmp_sql_server.is_connect_success:
                    ...
                    mia.get_goods_data(goods_id=item[1])
                    ...
                    if data != {}:
                        ...
                        if data.get('is_delete') == 1:  # 单独处理下架商品
                            my_lg.info('@@@ 该商品已下架...')
                            tmp_sql_server._update_table_2(sql_str=mia_update_str_5, params=(item[1],), logger=my_lg)
                            sleep(TMALL_REAL_TIMES_SLEEP_TIME)
                            continue
                        else:
                            price_info_list = old_sku_info = json_2_dict(item[7], default_res=[])
                            try:
                                old_sku_info = format_price_info_list(price_info_list=price_info_list, site_id=32)
