...
def daemon_init(stdin='/dev/null', stdout='/dev/null', stderr='/dev/null'):
    ...
    os.setsid()
    '''防止在类似于临时挂载的文件系统下运行, 例如/mnt文件夹下, 这样守护进程一旦运行, 临时挂载的文件系统就无法卸载了, 这里我把当前工作目录切换到根目录下'''
    os.chdir("/")
    ...
    # 孙进程
    #   for i in range(3, 64):  # 关闭所有可能打开的不需要的文件，UNP中这样处理，但是在python中实现不需要。
    #       os.close(i)
    sys.stdout.write("Daemon has been created! with pid: %d\n".format(os.getpid()))
    sys.stdout.flush()  # 由于这里我使用的是标准IO，这里应该是行缓冲或全缓冲，因此要调用flush，从内存中刷入日志文件。
