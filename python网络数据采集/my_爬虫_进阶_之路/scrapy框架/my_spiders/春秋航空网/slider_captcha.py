...
def fake_drag(driver, knob, offset):
...
def do_crack(driver):
    slice = driver.find_element_by_class_name("gt_slice")
    slice_offset = get_slice_offset(slice)
    print(slice_offset)

    images = driver.find_elements_by_class_name("gt_cut_fullbg_slice")
    images_bg = driver.find_elements_by_class_name("gt_cut_bg_slice")
    ...
    # ActionChains(driver).drag_and_drop_by_offset(knob, offset - slice_offset, 0).perform()
    fake_drag(driver, knob, offset - slice_offset)
    return
...
email = driver.find_element_by_css_selector("div[data-target='account-login']")
email.click()

Wait(driver, 60).until(
    Expect.visibility_of_element_located((By.ID, "emailRegist"))
)
driver.find_element_by_name('EmailInput').send_keys('11111@qq.com')
driver.find_element_by_name('EmailPasswordInput').send_keys('11111111a')
driver.find_element_by_name('EmailPasswordConfirm').send_keys('11111111a')

register = driver.find_element_by_id("emailRegist")
register.click()
