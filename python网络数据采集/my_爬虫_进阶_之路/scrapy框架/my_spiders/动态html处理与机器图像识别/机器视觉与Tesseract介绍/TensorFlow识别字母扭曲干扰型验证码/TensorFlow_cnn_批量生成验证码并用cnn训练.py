...
# 由于我们需要使用深度学习模型进行训练，所以这里我们的 label 数据最好使用 One-Hot 编码
# 即如果验证码文本是 1234，那么应该词表索引位置置 1，总共的长度是 40，我们用程序实现一下 One-Hot 编码和文本的互相转换
# 独热编码即One-Hot 编码，又称一位有效编码，其方法是使用N位状态寄存器来对N个状态进行编码，每个状态都由他独立的寄存器位，并且在任意时候，其中只有一位有效
def text2vec(text):
    '''
    文本转向量(文本转为One-Hot 编码)
    :param text:
    :return:
    '''
    ...
    for i, c in enumerate(text):    # 返回index, item
...
def train_crack_captcha_cnn():
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            print(step, loss_)  # loss_ 100%的小数表示
