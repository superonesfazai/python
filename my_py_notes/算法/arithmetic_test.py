...
#在python2.7测试通过,在python3测试未通过
def add(x, y):
    """
    加法
    """
    result = [int(p)+int(q)
              for i_p, p in enumerate(('0'*(max(len(x), len(y))-len(x))+str(x))[::-1])
              for i_q, q in enumerate(('0'*(max(len(x), len(y))-len(y))+str(y))[::-1]) if i_p == i_q]
...
if __name__ == '__main__':
    ...
    test_multiply()
    print('')
    print('测试逆波兰式'.center(45, '*'))
    # test_get_reversed_polish()
    print('')
    print('表达式测试'.center(45, '*'))
    test_expression()
    print('')
